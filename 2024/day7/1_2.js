...
const checkCombinations = (numbersToCalculate, target) => {
  for (let i = 0; i < Math.pow(3, numbersToCalculate.length - 1); i++) {
    let result = numbersToCalculate[0];
    let operationLoop = i;

    for (let j = 0; j < numbersToCalculate.length - 1; j++) {
      const operation = operationLoop % 3;
      operationLoop = Math.floor(operationLoop / 3);
      ...
      if (operation === 0) {
        ...
        result = Number(`${result}${numbersToCalculate[j + 1]}`);
      }
    }
